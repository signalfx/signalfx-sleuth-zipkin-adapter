...
/**
 * <p>
 * A copy and modify of <code>org.springframework.cloud.sleuth.zipkin.HttpZipkinSpanReporter:1.0.6.RELEASE</code>
 * </p><p>
 * This class should only differ in the URL endpoint being used.  Zipkin listens on the <code>api/v2/spans</code>
 * endpoint and thus had the endpoint value hardcoded.  SignalFx Smart Gateway listens on
 * <code>/v1/trace</code>. This change is made in the constructor.
 * </p>
 * 
 * @author brandon
 * @see org.springframework.cloud.sleuth.zipkin.HttpZipkinSpanReporter
 */
public class SignalFxZipkinReporter implements ZipkinSpanReporter, Flushable, Closeable {

    private static final Log log = org.apache.commons.logging.LogFactory
	    .getLog(SignalFxZipkinReporter.class);
     ...
     */
    public SignalFxZipkinReporter(String baseUrl, int flushInterval, boolean compressionEnabled,
	    SpanMetricReporter spanMetricReporter) {
	// Zipkin listens on the api/v2/spans endpoint, which was hardcoded.  SignalFx SmartGateway listens on /v1/trace
	this.url = baseUrl + (baseUrl.endsWith("/") ? "" : "/") + "v1/trace";
	this.flusher = flushInterval > 0 ? new Flusher(this, flushInterval) : null;
	this.compressionEnabled = compressionEnabled;
	this.spanMetricReporter = spanMetricReporter;
	log.info("Registering Zipkin reporter for SignalFx endpoint: " + this.url);
    }
